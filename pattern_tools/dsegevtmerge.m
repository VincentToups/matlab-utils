function dseg = dsegevtttest(spikes,dseg)
% DSEGEVTTTEST decides whether two events are the same base on a ttest.
%
% DSEG = DSEGEVTTTEST(DSET) decides two events are the same by doing 
%  a ttest at 5% confidence.
%
% See also DSEGEVTMERGE which is the same function but with a slightly 
%   different internal, allowing the specification of the ttest percentage.
%   this all needs to be folded into the main code.
...
for i=1:ll
    for j=i:ll
        sm(i,j) = ttest2(ttevents{i},ttevents{j});
    end
end
sm = sm + sm';
sm = ~sm;
