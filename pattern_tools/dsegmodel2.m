...
for i=1:length(utc)
    ...
    tt = trialuu(tchosen==i,:,1); % Get the trial indices of just these spikes
    ...
    tt = ss(tt>0);
    ss = ss(ss>0);
    assert(length(stc)==length(ss),'Woah! length(stc)~=length(ss)');
    ustc = unique(stc); % Get the indices for the time clusterings
    assert(ustc==1:length(ustc),'stc needs to be relabeled, apparently');
    for j=1:length(ustc)
