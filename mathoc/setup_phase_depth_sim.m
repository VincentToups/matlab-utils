function [voltage_traces_file,spikes_file]=setup_phase_depth_sim(phase,depth,ereli,estd,rebuild)
...
voltage_traces_file = sprintf('data/osctraces_%f_%f_%f_%f.trace',phase,depth,ereli,estd);
spikes_file = sprintf('data/nullspikes_%f_%f_%f_%f.spikes',phase,depth,ereli,estd);
...
if rebuild
  ...
  exc_env = @(t) 1/1000 + (depth)*sin(2*pi*(10/1000)*t+phase);
